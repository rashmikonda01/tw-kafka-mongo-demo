...
app.layout = html.Div(children=[
    html.Div(create_header('Follow Your Tweet - Dashboard')),
    html.Div([
        html.Div(
            dcc.Graph(
...
def update_graph_live(n):
    ...
    data = [trace0]
    layout = dict(title='<b>Most Recent Tweets </b>', height=700,
...
def most_active_users(n):
    ...
    df2['location'] = df2['location'].dropna()
    df2['usercount'] = df2.groupby('location')['location'].transform('count')
    result1 = df2[['location', 'usercount']].drop_duplicates()
    result1 = result1.sort_values(by=['usercount'], ascending=False).head(10)
    ...
    data1 = [go.Bar(
        ...
        text=result1['usercount'],
        orientation='h',
    )]
    layout = go.Layout(dict(title='<b>Most Active Users Tweeting by Geo</b>', height=700,
                  titlefont=dict(size=20)),yaxis=dict(showticklabels=True, automargin=True))
